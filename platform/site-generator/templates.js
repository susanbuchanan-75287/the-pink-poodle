/**
 * Pink Poodle Platform — shared site templates (isomorphic)
 * --------------------------------------------------------------------------
 * The single source of truth for a generated tenant site. Runs in BOTH:
 *   - Node (the CLI generator, generate.js) — writes files to disk.
 *   - The browser (the setup wizard, wizard/index.html) — live preview.
 * Because both consume these exact functions, the wizard preview a groomer
 * sees is byte-for-byte what gets published. Zero runtime dependencies.
 * --------------------------------------------------------------------------
 */
"use strict";
...
function jsonForScript(v, indent) {
  return JSON.stringify(v, null, indent)
    .replace(/</g, "\\u003c").replace(/>/g, "\\u003e").replace(/&/g, "\\u0026")
    .replace(/\u2028/g, "\\u2028").replace(/\u2029/g, "\\u2029");
}
/** Validate a CSS hex color, falling back to a default. */
...
function normalize(cfg) {
  ...
  return {
    ...
    brand: {
      ...
      ink: hexColor(brand.ink, "#221820"),
      cream: hexColor(brand.cream, "#fdf6fa"),
    },
  };
}
...
<script>
(function () {
  var ENDPOINT = ${jsonForScript(t.bookingEndpoint)};
  var SMS = ${jsonForScript(t.sms)};
  var EMAIL = ${jsonForScript(t.email)};
...
/**
 * Build every page for a tenant. Accepts a RAW config (normalizes internally)
 * and returns a { filename: contents } map — no filesystem side effects, so
 * the browser wizard and the Node CLI share this exact output.
 */
function buildFiles(cfg) {
  const t = normalize(cfg);
  return {
...
const PPGen = {
  esc, money, req, normalize,
  head, nav, footer,
  pageHome, pageServices, pageAbout, pageBook, styles,
  buildFiles,
};

if (typeof module !== "undefined" && module.exports) module.exports = PPGen;
if (typeof window !== "undefined") window.PPGen = PPGen;
